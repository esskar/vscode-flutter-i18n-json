import { commands, window, ExtensionContext, languages } from "vscode";
...
export function activate(context: ExtensionContext) {
  ...
  const i18nGenerator = new I18nGenerator(
    ...
    rootFolder,
    new FileSystem(),
    new UserActions(window)
  );

  subscriptions.push(
    commands.registerCommand("flutterI18nJsonInit", () => {
      i18nGenerator.generateInitializeAsync();
    })
  );

  subscriptions.push(
    commands.registerCommand("flutterI18nJsonAdd", () => {
      i18nGenerator.generateAddAsync();
    })
  );

  subscriptions.push(
    commands.registerCommand("flutterI18nJsonUpdate", () => {
      i18nGenerator.generateUpdateAsync();
    })
  );

  subscriptions.push(
    commands.registerCommand("flutterI18nJsonRemove", () => {
      i18nGenerator.generateRemoveAsync();
    })
  );

  subscriptions.push(
    commands.registerCommand("flutterI18nGTranslateApiCodeAdd", () => {
      i18nGenerator.generateGTranslateApiCodeAdd();
    })
  );

  subscriptions.push(
    commands.registerCommand("flutterI18nAutoTranslate", () => {
  ...
  subscriptions.push(
    languages.registerCodeActionsProvider(provider.filter, provider)
  );

  subscriptions.push(
    commands.registerCommand(provider.actionName, (input) => {

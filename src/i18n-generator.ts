...
export class I18nGenerator
  implements IDisposable, InsertActionProviderDelegate
{
  ...
  async generateInitializeAsync(): Promise<void> {
    ...
    let customRootFolder = await this.ua.promptAsync(
        "Enter root folder path (or leave empty if it is in root is workspace",
        "",
        this.validateRootFolder
        );

    if(!customRootFolder) {
        customRootFolder = this.workspaceFolder;
    }
    else {
        this.rootFolder = path.join(this.workspaceFolder, customRootFolder);
    }
    ...
    this.ua.showInfo(
      `Successfully initialized localization with default locale '${defaultLocale}'.`
    );
  }
  ...
  async generateRemoveAsync(): Promise<void> {
    const config = await this.readConfigFileAsync();
    let pickedLocale = await this.ua.pickAsync(
      "Remove existing locale code",
      config.locales
    );
    if (!pickedLocale) {
      return;
  ...
  private async initializeAsync(config: I18nConfig): Promise<void> {
    await this.fs.createFolderAsync(
      this.fs.combinePath(this.rootFolder, config.generatedPath)
    );
    await this.fs.createFolderAsync(
      this.fs.combinePath(this.rootFolder, config.localePath)
    );
  }
  ...
  private async generateDartFileAsync(config: I18nConfig): Promise<void> {
    ...
    dartContent += this.generateLocales(
      I18nGenerator.dartGeneratedLocalizationsDelegate,
      config
    );

    const path = this.fs.combinePath(
      this.rootFolder,
  ...
  async readConfigFileAsync(): Promise<I18nConfig> {
    const filename = this.fs.combinePath(
      this.rootFolder,
      I18nGenerator.i18nConfigFile
    );
    return await this.fs.readJsonFileAsync<I18nConfig>(filename);
  }

  async readI18nFileAsync(
    locale: string,
    config: I18nConfig,
    noFlat?: boolean
  ): Promise<{ [id: string]: any }> {
    const path = this.fs.combinePath(this.rootFolder, config.localePath);
  ...
  private async removeI18nFileAsync(
    locale: string,
    config: I18nConfig
  ): Promise<void> {
    const path = this.fs.combinePath(this.rootFolder, config.localePath);
  ...
  private async generateAutoTranslationsSetAsync(
    ...
    for (let availableTranslation of availableKeys) {
      ...
      if (typeof value === "object") {
        ...
        if (
          !(await this.generateAutoTranslationsSetAsync(
            translator,
            locale,
            Object.keys(value),
            value,
  ...
  private async generateAutoTranslationsAsync(
    config: I18nConfig
  ): Promise<boolean> {
      ...
      if (locale === config.defaultLocale) {
        ...
        try {
          ...
          const translations: { [id: string]: any } =
            await this.readI18nFileAsync(locale, config, true);

          const result = await this.generateAutoTranslationsSetAsync(
            translator,
            locale,
            availableKeys,
            defaultI18n,
          ...
          await this.ua.showError(
            `Failed to build translations for ${locale}: ${e}`
          );
  ...
  private validateRootFolder = (root:string): string | null => {
    const rootFull = path.join('./', this.workspaceFolder, root);
    //const rootFull = this.fs.combinePath('./', this.workspaceFolder, root);

    if (!fileSystem.existsSync(rootFull)) {
      return `Folder ${rootFull} does not exist.`;
    }
    
    if (!fileSystem.lstatSync(rootFull).isDirectory()) {
      return `Folder ${rootFull} is not a directory.`;
    }
    ...
    for (const property in obj) {
      ...
      if (value instanceof Object && !(value instanceof Array)) {
        ...
        for (const subProperty in flattenedSubObject) {
          ...
          result[property + subProperty.upperCaseFirstLetter()] =
            Object.values(flattenedSubObject)[subIndex];
          subIndex++;
        }
      } else {
